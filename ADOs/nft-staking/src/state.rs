...
use cosmwasm_std::{Addr, BlockInfo, Storage, Uint64};
...
pub struct AssetDetail {
    ...
    pub token_id: String,
    pub unbonding_period: Milliseconds,
    pub pending_rewards: Uint64,
    pub last_payout: Milliseconds,
    pub unstaked_at: Option<Milliseconds>,
}

pub const CONFIG: Item<Config> = Item::new("config");

pub const REWARDS_PER_TOKEN: Map<&str, Uint64> = Map::new("rewards_per_token");
...
pub fn set_rewards_per_token(
    ...
    rewards_per_token: Vec<(String, u64)>,
...
pub fn get_rewards_per_token(store: &dyn Storage) -> Result<Vec<(String, u64)>, ContractError> {
...
pub fn calculate_pending_rewards(
    ...
    let unpaid_duration = block
        .time
        .nanos()
        .checked_sub(asset_detail.last_payout.nanos())
        .unwrap_or_default();
    let config = CONFIG.load(store).unwrap_or_default();
    let window_count = Uint64::new(unpaid_duration / config.payout_window.nanos());
    ...
    window_count
        ...
        .unwrap_or(asset_detail.pending_rewards)
}
